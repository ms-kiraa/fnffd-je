...
/**
 * The base note class. Comes with variables for controlling notes and utility methods
 */
public class Note extends GameObject {
    ...
    public Note(double x, double y, int direction, Camera camera, boolean fump, boolean playerNote){
        super(x, y, 1.3, new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB), camera);
        this.dir = getIntAsDirection(direction);
        this.playerNote = playerNote;
        this.fumpNote = fump;
    }
     ...
     * @return 
     */
    protected String getDirectionAsString(CapsMode capsMode){
    ...
    protected Direction getIntAsDirection(int direction){
        switch(direction){
            case 0:
    ...
    /**
     * An enum to hold all the possible directions a note can have
     */
